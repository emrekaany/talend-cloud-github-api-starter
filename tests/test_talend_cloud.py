...
from talend_api_starter.talend_cloud import (
    TALEND_API_VERSION,
    TALEND_BASE_URL_ENV,
    TALEND_TOKEN_ENV,
    TalendCloudClient,
...
def test_workspaces_is_get_only_and_sends_pat_in_header() -> None:
    ...
    with TalendCloudClient.from_env(transport=httpx.MockTransport(handler)) as client:
...
def test_remote_errors_are_classified_and_redacted(
    ...
    with (
        TalendCloudClient.from_env(transport=transport) as client,
...
def test_redirect_is_refused_without_second_request() -> None:
    ...
    with (
        TalendCloudClient.from_env(transport=httpx.MockTransport(handler)) as client,
...
def test_transport_rejects_absolute_target_before_forwarding_token() -> None:
    ...
    with TalendCloudClient.from_env(
...
def test_cloud_query_validation_happens_before_network(
    call: Callable[[TalendCloudClient], Any],
) -> None:
    def fail_if_called(_: httpx.Request) -> httpx.Response:
        raise AssertionError("network should not be called")
...
def test_wrong_content_type_is_rejected_but_absent_header_is_allowed() -> None:
    ...
    with TalendCloudClient.from_env(transport=transport) as client:
...
def test_decompressed_response_bytes_are_bounded() -> None:
    ...
    with (
        TalendCloudClient.from_env(
            max_response_bytes=200, transport=transport
        ) as client,
